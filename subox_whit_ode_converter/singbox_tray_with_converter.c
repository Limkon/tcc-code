...
void StartSingBox() {
    ...
    // 在父进程中关闭管道的写入端
    // 这很重要，这样ReadFile才不会无限期阻塞
    CloseHandle(hChildStd_OUT_Wr);

    // 检查进程是否迅速退出
    if (WaitForSingleObject(pi.hProcess, 500) == WAIT_OBJECT_0) { // 进程在500毫秒内终止
        char chBuf[4096] = {0};
        DWORD dwRead = 0;
